...
```{r}
list(1L:3L, c("a", "bc"), c(3.14, 123, 999))
...
## Subsetting: "subsetting" versus "item extraction"

When subsetting non-atomic data types (e.g., lists), subsetting operations may be distinguished by whether they return:

- An subsetted object of the same type (e.g., another list)

- An simpler object that was contained in the original object (e.g., a vector element stored inside a list)

For example, if `z` is a list:

- `z[1]` returns a sub-list of length 1, containing only the first element of `z`
